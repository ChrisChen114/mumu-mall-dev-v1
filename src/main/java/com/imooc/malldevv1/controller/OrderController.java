...
public class OrderController {
    ...
    //入参；从购物车中查找已经勾选的商品；判断商品是否正在售卖中；判断库存，保证不超卖，扣库存；数据库事务；删除购物车中对应的商品；生成订单；订单号生成规则；循环保存每个商品到order_item表；
    ...
    public ApiRestResponse create(@Valid @RequestBody CreateOrderReq createOrderReq){
        //s0，新建CreateOrderReq请求参数类，含receiverName，receiverMobile，和receiverAddress，以及带默认参数的postage和paymentType
